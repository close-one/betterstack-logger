...
class BetterStackLogger
{
  ...
  {
    $event_messages = [
      "wp_login" => isset($args[0])
        ? "User {$args[0]} logged in."
        : "Login event detected, but user data is missing.",
      "user_register" => ($user = get_userdata($args[0]))
        ? "New user registered: {$user->user_login}."
        : "User registration event detected, but user data is missing.",
      "delete_user" => ($user = get_userdata($args[0]))
        ? "User deleted: {$user->user_login}."
        : "User deletion event detected, but user data is missing.",
      "profile_update" => ($user = get_userdata($args[0]))
        ? "User profile updated: {$user->user_login}."
        : "User profile update detected, but user data is missing.",
      "password_reset" =>
        isset($args[0]) && is_object($args[0])
          ? "User {$args[0]->user_login} reset their password."
          : "Password reset event detected, but user data is missing.",
      "transition_post_status" => $this->get_post_status_message($args),
      "delete_post" => ($post = get_post($args[0]))
        ? "Post deleted: '{$post->post_title}' (ID: {$post->ID})."
        : "Post deletion event detected, but post data is missing.",
      "save_post" => $this->get_save_post_message($args),
      "activated_plugin" => $this->get_plugin_name($args[0])
        ? "Plugin activated: " . $this->get_plugin_name($args[0]) . "."
        : "Plugin activation event detected, but plugin data is missing.",
      "deactivated_plugin" => $this->get_plugin_name($args[0])
        ? "Plugin deactivated: " . $this->get_plugin_name($args[0]) . "."
        : "Plugin deactivation event detected, but plugin data is missing.",
      "switch_theme" => isset($args[0])
        ? "Theme switched to: {$args[0]}."
        : "Theme switch event detected, but theme data is missing.",
  ...
  private function create_log_entry($message)
  {
    ...
    if (is_wp_error($response)) {
      return "Failed to send log message. Error: " .
  ...
  public function betterstack_wp_die_handler()
  {
    return function ($message, $title = "", $args = []) {
      $error_logging_enabled = get_option(
        "betterstack_error_logging_enabled",
        "yes",
      );
      if ($error_logging_enabled === "yes") {
        // Sanitize the message before logging - handle both string and WP_Error
        if (is_wp_error($message)) {
          $sanitized_message = wp_strip_all_tags($message->get_error_message());
        } else {
          $sanitized_message = wp_strip_all_tags((string) $message);
        }
        $this->log_error($sanitized_message);
      }
  ...
  public function settings_page()
  {
    if (isset($_POST["betterstack_test_message"])) {
      ...
      if (
        !isset($_POST["betterstack_nonce"]) ||
        !wp_verify_nonce(
          sanitize_text_field(wp_unslash($_POST["betterstack_nonce"])),
          "betterstack_test_message_action",
        )
      ) {
      ...
      $test_message = sanitize_text_field(
        wp_unslash($_POST["betterstack_test_message"]),
      );
  ...
  public function settings_init()
  {
    ...
    add_settings_field(
      ...
      function () {
        if (defined("BETTERSTACK_API_KEY")) {
          $masked_key =
            substr(BETTERSTACK_API_KEY, 0, 4) .
            str_repeat("*", max(0, strlen(BETTERSTACK_API_KEY) - 8)) .
            substr(BETTERSTACK_API_KEY, -4);
    ...
    add_settings_field(
      ...
      function () {
        ...
        echo '<input type="checkbox" name="betterstack_error_logging_enabled" value="yes" ' .
          checked($error_logging_enabled, "yes", false) .
    ...
    add_settings_field(
      ...
      function () {
        $event_logging_enabled = get_option(
          "betterstack_event_logging_enabled",
          "no",
        );
        echo '<input type="checkbox" name="betterstack_event_logging_enabled" value="yes" ' .
